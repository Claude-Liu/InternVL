...
class ModelArguments:
    ...
    apply_moco: bool = field(
        default=False, metadata={'help': 'Set to True to enable the use momentum contrastive learning strategy in moco.'},
    )
...
def main():
    ...
    # Here you need to specify the path to your weight files
    weights_files = [
        "{}/pytorch_model-00001-of-00003.bin".format(model_args.model_name_or_path),
        "{}/pytorch_model-00002-of-00003.bin".format(model_args.model_name_or_path),
        "{}/pytorch_model-00003-of-00003.bin".format(model_args.model_name_or_path),
    ]

    # Step 4: Load weights into the model
    # If the weights are split across multiple files, you'll need to combine them
    state_dict = {}
    ...
    if model_args.freeze_model:
        ...
        # unfreeze the query tokens
        #model.query_tokens.requires_grad = True
        # unfreeze the logit scale
        #model.logit_scale.requires_grad = True
        # unfreeze the clip_projector2 for embeddings of query tokens
        _unfreeze_params(model.clip_projector2)
        # unfreeze the itm head
        _unfreeze_params(model.itm_head)
    ...
    model.qllama.print_trainable_parameters()
    if dist.get_rank() == 0:
        for name, param in model.named_parameters():
