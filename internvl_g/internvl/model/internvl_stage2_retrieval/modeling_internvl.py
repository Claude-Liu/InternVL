...
class GatherLayer(torch.autograd.Function):
    """Gather tensors eom all process, supporting backward propagation.
    """
...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def __init__(self, config: InternVLConfig, moco=False, queue_size=8192):
        ...
        self.moco = moco
        self.queue_size = None # overloaded later
        ...
        if self.moco:
            ...
            self.text_projection_m = nn.Parameter(torch.empty(text_hidden_size, clip_embed_dim))  # frozen
            self.logit_scale_m = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))  # trainable
            self.clip_projector_m = AttentionPoolingBlock(  # frozen
                dim=vision_hidden_size, num_heads=attn_pool_num_heads, qkv_bias=True, qk_scale=None,
                drop=0., attn_drop=0., norm_layer=partial(nn.LayerNorm, eps=1e-5), out_dim=clip_embed_dim)
            self.clip_projector2_m = AttentionPoolingBlock(  # trainable
                dim=text_hidden_size, num_heads=attn_pool_num_heads, qkv_bias=True, qk_scale=None,
                drop=0., attn_drop=0., norm_layer=partial(nn.LayerNorm, eps=1e-5), out_dim=clip_embed_dim)
            self.itm_head_m = nn.Linear(text_hidden_size, 2)  # trainable
            self.model_pairs = [
                (self.vision_model, self.vision_model_m),
                (self.qllama, self.qllama_m),
                (self.query_tokens, self.query_tokens_m),
                (self.text_projection, self.text_projection_m),
                (self.logit_scale, self.logit_scale_m),
                (self.clip_projector, self.clip_projector_m),
        ...
        if config.force_image_size:
            ...
            if self.moco:
                self.vision_model_m.resize_pos_embeddings(
                    old_size=config.vision_config.image_size,
                    new_size=config.force_image_size,
                    patch_size=config.vision_config.patch_size
                )
            print("the shape of the image position embedding of vision_model is {}".format(self.vision_model.embeddings.position_embedding.shape))
            if self.moco:
    ...
    def wrap_backbone_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        if self.moco:
            self.vision_model_m = get_peft_model(self.vision_model_m, lora_config)
    ...
    def wrap_qllama_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        if self.moco:
            self.qllama_m = get_peft_model(self.qllama_m, lora_config)
        self.qllama.print_trainable_parameters()

    def get_input_embeddings(self):
        return self.qllama.get_input_embeddings()
    
    def get_input_embeddings_m(self):
        if self.moco:
            return self.qllama_m.get_input_embeddings()
        else:
            return None
    ...
    def forward(
        ...
        if self.moco:
            ...
            backbone_embeds_all = torch.cat([backbone_embeds_m, self.image_backbone_queue.clone().detach()], dim=0)
            image_itc_all = torch.cat([image_itc_m, self.image_queue.clone().detach()], dim=0)
            text_itc_all = torch.cat([text_itc_m, self.text_queue.clone().detach()], dim=0)
        else:
        ...
        if self.moco:
            # dequeue and enqueue
            self._dequeue_and_enqueue(image_itc_m, backbone_embeds_m, text_itc_m, image_ids)
            #self._momentum_update()
                    ...
                    if type(model_pair[0]) == nn.Parameter:
                        ...
                        for (name, param), (name_m,param_m) in zip(model_pair[0].named_parameters(), model_pair[1].named_parameters()):
                            try:
                                ...
                                print("the device of the param is {}".format(param.device))
                                print("------------------")
                                print("the shape of param_m is {}".format(param_m.shape))
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feat, image_backbone_feature, text_feat, idx):
        # gather keys before updating queue
        image_feats = concat_all_gather(image_feat)
        image_backbone_feats = concat_all_gather(image_backbone_feature)
        ...
        self.image_queue[ptr:ptr + batch_size] = image_feats
        self.image_backbone_queue[ptr:ptr + batch_size] = image_backbone_feats
